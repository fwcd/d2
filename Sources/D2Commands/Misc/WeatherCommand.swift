...
public class WeatherCommand: StringCommand {
    ...
    public func invoke(with input: String, output: any CommandOutput, context: CommandContext) async {
        guard let city = input.nilIfEmpty ?? config.city else {
            await output.append(errorText: "Please enter a city name (or configure a city)!")
            return
        }

        do {
            let weather = try await OpenWeatherMapQuery(city: city).perform()
            ...
            await output.append(error, errorText: "Could not fetch the weather")
        }
    }
}
