...
public class SolveLinearSystemCommand: Command {
    ...
    public func invoke(with input: RichValue, output: any CommandOutput, context: CommandContext) async {
        ...
        guard matrix.width == matrix.height + 1 else {
            await output.append(errorText: "The matrix should have the shape of an augmented matrix (i.e. be one column wider than a square matrix).")
            return
        }
        ...
        guard let aInverted = a.inverse else {
            await output.append(errorText: "Linearly dependent equations are currently not supported")
            return
        }

        await output.append(.ndArrays([(aInverted * b).asNDArray]))
    }
}
