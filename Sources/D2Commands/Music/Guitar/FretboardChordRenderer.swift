...
struct FretboardChordRenderer: ChordRenderer {
    ...
    func render(chord: Chord) throws -> CairoImage {
        ...
        graphics.draw(rect: Rectangle(topLeft: topLeft - Vec2(y: gutterHeight), size: Vec2(x: innerWidth, y: gutterHeight), color: fgColor, isFilled: true))

        for stringIndex in 0..<stringCount {
            let position = topLeft + Vec2(x: stringSpacing * Double(stringIndex))
            graphics.draw(line: LineSegment(from: position, to: position + Vec2(y: innerHeight), color: fgColor))
        }

        for fretIndex in 0..<fretCount {
            let position = topLeft + Vec2(y: fretSpacing * Double(fretIndex))
            graphics.draw(line: LineSegment(from: position, to: position + Vec2(x: innerWidth), color: fgColor))
        }
        ...
        for dot in guitarChord.dots {
            ...
            if dot.fret > 0 {
                ...
                graphics.draw(ellipse: Ellipse(center: position, radius: Vec2(both: dotRadius), color: fgColor))
            } else {
                let position = topLeft + Vec2(x: dotX, y: -gutterHeight - dotRadius)
                graphics.draw(ellipse: Ellipse(center: position, radius: Vec2(both: dotRadius), color: fgColor, isFilled: false))
            }
        }
